...
class DbRunEventStore(RunEventStore):
    ...
    def _row_to_dict(row: RunEventRow) -> dict:
        ...
        if isinstance(raw, str) and (
            d.get("metadata", {}).get("content_is_json")
            or d.get("metadata", {}).get("content_is_dict")
        ):
            try:
    ...
    def _truncate_trace(self, category: str, content, metadata: dict | None) -> tuple[object, dict]:
        if category == "trace":
            text = json.dumps(content, default=str, ensure_ascii=False) if isinstance(content, (dict, list)) else content
    ...
    @staticmethod
    def _serialize_content(content, metadata: dict | None) -> tuple[str, dict]:
        if isinstance(content, (dict, list)):
            return json.dumps(content, default=str, ensure_ascii=False), {
                **(metadata or {}),
                "content_is_json": True,
            }
        return str(content), metadata or {}
    ...
    async def put(self, *, thread_id, run_id, event_type, category, content="", metadata=None, created_at=None):  # noqa: D401
        ...
        db_content, metadata = self._serialize_content(content, metadata)
