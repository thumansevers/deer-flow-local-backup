...
export function MessageGroup({
  ...
  useEffect(() => {
    if (!isLoading || !lastReasoningStep) {
      if (!isLoading) {
        autoOpenedReasoningRef.current = null;
      }
      return;
    }

    const reasoningKey = lastReasoningStep.id ?? lastReasoningStep.messageId;
    if (reasoningKey && autoOpenedReasoningRef.current !== reasoningKey) {
      setShowLastThinking(true);
      autoOpenedReasoningRef.current = reasoningKey;
    }
  }, [isLoading, lastReasoningStep]);
